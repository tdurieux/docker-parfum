...
RUN groupadd -r postgres && useradd -r --no-log-init -g postgres postgres \
  ...
  && curl -f -L -s https://packages.2ndquadrant.com/bdr/apt/AA7A6805.asc | apt-key add - \
  && curl -f -L -s https://www.postgresql.org/media/keys/ACCC4CF8.asc | apt-key add - \
